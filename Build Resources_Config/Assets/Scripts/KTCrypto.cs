...
/// <summary>
/// Lớp mô tả mã hóa chuỗi Byte tài nguyên của KT
/// </summary>
public static class KTResourceCrypto
{
    /// <summary>
    /// Chuyển chuỗi thành ByteArray
    /// </summary>
    ...
    {
        KTResourceCrypto.Key = KeyValue;
        KTResourceCrypto.KeyBytesArray = KTResourceCrypto.StringToByteArray(KTResourceCrypto.Key);
    }
    ...
    public static byte[] Decrypt(byte[] buffer)
    {
        ...
        for (int j = 0; j < len; ++j)
        {
            if (j % 2 != 0)
            {
                byte temp = buffer[j + offset];
                buffer[j + offset] = buffer[j + offset + len];

                int vitri = j + offset + len;

                buffer[vitri] = temp;
            }
        }
        int len2 = n - offset + -3 * len;
        for (int j = 0; j < len2; ++j)
        {
            if (j % 2 == 0)
            {
                byte temp = buffer[offset + 2 * len + j];
                buffer[offset + 2 * len + j] = buffer[offset + 3 * len + j];
                buffer[offset + 3 * len + j] = temp;
            }
        }
        int len3 = n - offset;
        int i = 0;
        ...
        {
            byte temp = buffer[offset + j];
            int off = i++;
            buffer[offset + j] = (byte) (temp ^ key[off]);
            if (i % k == 0)
                i = 0;
        }
    ...
    /// <summary>
    /// Mã hóa chuỗi byte array theo công thức đảo ngược bit 
    /// <para>Ưu điểm nhẹ và tốn ít tài nguyên</para>
    /// </summary>
    /// <param name="buffer"></param>
    /// <returns></returns>
